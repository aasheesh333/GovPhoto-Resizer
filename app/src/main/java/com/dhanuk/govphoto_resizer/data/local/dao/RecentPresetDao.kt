...
interface RecentPresetDao {
    ...
    suspend fun upsert(item: RecentPresetEntity)
    ...
    @Query("UPDATE recent_presets SET useCount = useCount + 1, usedAt = :now WHERE presetId = :presetId")
    suspend fun bumpUse(presetId: String, now: Long = System.currentTimeMillis())
